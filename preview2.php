<?php
    ...
    if (isset($_POST["preview"]) &&$_POST["preview"]!="") {
        ...
        try {
            ...
            while (isset($_FILES[$file_id]['error']) && is_int($_FILES[$file_id]['error'])) {
                ...
                try {
                    ...
                    $bool_add_galley = "galley".$file_id;
                    ...
                    $_SESSION["img"][$img_number]["galley_flag"] = $_FILES[$file_id][$bool_add_galley];
        ...
        //画像を文章間に挿入するための分割処理
        $divide_article[0] = $tmp_article_content = nl2br($_POST["detail"]);
        ...
        for ($file_num=1;$file_num<=5;$file_num++) {
            ...
            if (strstr($tmp_article_content, $img_position_string)) {
                ...
                $divide_article[$file_num] = $pieces[0];
                $tmp_article_content = $pieces[1];
                //echo $pieces[0]."<br /><br />";
                //echo $pieces[1]."<br /><br />";
            }
            else {
                $divide_article[$file_num] = $tmp_article_content;
                break;
            }
        } 
        //送信データをセッションに保存
        $_SESSION["article_title"] = $_POST["title"];
        $_SESSION["article_content"] = nl2br($_POST["detail"]);
    }
	/*
    session_start();
...
<h1>プレビュー画面</h1>
    ...
    <?php
    for ($i=0;$i<=5;$i++) {
        if (isset($divide_article[$i])) {
            echo $i.":".$divide_article[$i]."<br/><br />";
        }
        else {
            break;
        }
